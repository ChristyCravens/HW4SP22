...
def LeastSquares(x, y, power):
    ...
    # Fill out the elements of the A matrix
...
def PlotLeastSquares(x, y, power, showpoints=True, npoints=500):
    ...
    yvals=np.array([Poly(x,a) for x in xvals])#$JES MISSING CODE HERE$#) #build the numpy array of yvals. I used a list comprehension with Poly(x, a) for x in xvals

    RSq = RSquared(x,y,a) #calculate an R^2 value
    pyplot.plot(xvals,yvals,linestyle='dashed',color='purple',linewidth='2')
...
def main():
    ...
    RSqLin=RSquared(x,y,ans1) #calculate RSquared for linear fit
    ...
    pyplot.plot(linx, liny, linewidth=2, linestyle='dashed', color='turquoise', label=r'Linear fit ($R^2={:0.3f}$)'.format(RSqLin)) #for the linear fit
    pyplot.plot(cubx, cuby, linewidth=2, linestyle='dotted', color='purple', label='Cubic fit ($R^2={:0.3f}$)'.format(RSqCub)) #for the cubic fit
