...
def main():
    ...
    p_a = nm.Probability(c_a, (mu_a, sig_a), GT=False)  # calculate the probability P(x<1|N(0,1))
    ...
    cdf_a = np.fromiter([nm.CDF(x, (mu_a, sig_a)) for x in x_a], float) #create a numpy array filled with values of CDF for each x in x_a.  I used a list comprehension.
    gnpdf_a = np.fromiter([nm.GNPDF(x) for x in x_a], float) #create a numpy array for f(x) from the GNPDF for each x in x_a.  I used a list comprehension.
    ...
    plt.plot(x_a, gnpdf_a) #plots the gndpf_a vs x_a
    ...
    x_fill = np.linspace(mu_a - 5 * sig_a, c_a, 100) #create a numpy array using linspace for x values from mu-5*sigma to c_a with 100 points
    gnpdf_fill = np.fromiter([nm.GNPDF(x) for x in x_fill], float) #calculate the GNPDF function for each x in x_fill and store in numpy array.  I used a list comprehension.
    ...
    plt.title('Part 1: P(x<1|N(0,1))') # add plot title
    ...
    plt.plot(c_a, p_a, 'o', markerfacecolor='white', markeredgecolor='red') #put a red circle on the cdf at location c_a, p_a
    ...
    cdf_b = np.fromiter([nm.CDF(x, (mu_b, sig_b)) for x in x_b], float) #create a numpy array filled with values of CDF for each x in x_b.  I used a list comprehension.
    gnpdf_b = np.fromiter([nm.GNPDF(x, (mu_b, sig_b)) for x in x_b], float) #create a numpy array for f(x) from the GNPDF for each x in x_b.  I used a list comprehension.

    # step 2: build the plots
    plt.subplots(2, 1, sharex=True)  # create two, stacked plots using subplots with sharex=True
    plt.subplot(2, 1, 1)  # set subplot 1 as our focus by using plt.subplot
    plt.plot(x_b, gnpdf_b) #plots the gndpf_a vs x_b
    ...
    x_fill = np.linspace(c_b, mu_b + 5 * sig_b, 100) #create a numpy array using linspace for x values from mu-5*sigma to c_b with 100 points
    gnpdf_fill = np.fromiter([nm.GNPDF(x, (mu_b, sig_b)) for x in x_fill], float) #calculate the GNPDF function for each x in x_fill and store in numpy array.  I used a list comprehension.
    ax = plt.gca()  # get the axes for the current plot
    ax.fill_between(x_fill, gnpdf_fill, color='grey', alpha=0.3)  # create the filled region between gnpdf and x axis
    ...
    plt.ylabel(r'$\Phi(x)=\int_{-\infty}^x f(x)dx$', size=12)  # label the y axis
    plt.xlabel('x')  # add the x label
    plt.plot(c_b, 1 - p_b, 'o', markerfacecolor='white', markeredgecolor='red') #put a red circle on the cdf at location c_b, p_b
    ax = plt.gca()  # get the current set of axes
    ax.tick_params(axis='both', which='both', direction='in', top=True, right=True, labelsize=10)  # format tick marks
