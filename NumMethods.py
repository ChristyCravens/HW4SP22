...
def GNPDF(x, args=(0,1)):
    ...
    f= (1/(s*sqrt(2*pi))) * exp(-0.5*((x-m)/s)**2) #calculate f(x)
    return f #return f(x)

def CDF(c=1, args=(0,1)):
    '''
    ...
    '''
    fn = lambda x: GNPDF(x, args) #WRITE A LAMBDA FUNCTION THAT CAN ACCEPT x AS AN ARGUMENT AND CALLS GNPDF(x,args)
    I, Err = integrate.quad(fn, 0-10*1, c) #USE integrate.quad TO RETURN THE VALUE OF THE INTEGRAL AND THE ERROR
    return I
